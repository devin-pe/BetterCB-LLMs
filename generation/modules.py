...
class CBL(nn.Module):
    def __init__(self, config, concept_dim, tokenizer, unsup_dim = None):
        super().__init__()
        if unsup_dim is None:
            unsup_dim = config.hidden_size

        self.cbl = nn.Linear(config.hidden_size, concept_dim)
        self.unsup = nn.Linear(config.hidden_size, unsup_dim)
        self.fc = nn.Linear(concept_dim + unsup_dim, config.vocab_size)
        self.relu = nn.ReLU()
