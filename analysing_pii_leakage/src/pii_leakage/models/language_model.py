...
class LanguageModel:
    ...
    def substring_perplexity(self, seq: str, substring: str) -> float:
        ...
        input_ids = torch.tensor(self._tokenizer.encode(txt, truncation=True)).unsqueeze(0)
        
        # For models with device_map (like distributed models), use the device of the first parameter
        if hasattr(self._lm, 'hf_device_map') and self._lm.hf_device_map:
            # Find the device of the first parameter for input placement
            device = next(self._lm.parameters()).device
            input_ids = input_ids.to(device)
        else:
            # Try to get model device, fallback to env_args.device if needed
            try:
                device = next(self._lm.parameters()).device
                input_ids = input_ids.to(device)
    ...
    def generate_batch(self, input_ids, attention_mask, sampling_args) -> List[GeneratedText]:
        ...
        # Tensors should already be on the correct device from the generate() method
    ...
    def generate(self, sampling_args: SamplingArgs) -> GeneratedTextList:
        ...
        # Move inputs to the appropriate device before generation
        # For models with device_map (like distributed models), use the device of the first parameter
        if hasattr(self._lm, 'hf_device_map') and self._lm.hf_device_map:
            # Find the device of the first parameter for input placement
            device = next(self._lm.parameters()).device
