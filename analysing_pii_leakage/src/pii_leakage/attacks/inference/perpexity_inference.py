...
class PerplexityInferenceAttack(ReconstructionAttack):
    ...
    def attack(self, lm: LanguageModel, target_sequence: str = None, pii_candidates: List[str] = None, *args, **kwargs):
        ...
        # 1.) Impute <MASK> tokens with error handling
        # For speed with large candidate sets, skip expensive mask filling
        if len(candidates) > 100:  # For large candidate sets, skip expensive mask filling
            if kwargs.get('verbose', False):
                print(f"Large candidate set ({len(candidates)}), using fast mask replacement")
            imputed_masked_sequence = masked_sequence.replace("<MASK>", "")
        else:
            try:
                imputed_masked_sequence = self._fill_masks.fill_masks(masked_sequence)
            except Exception as e:
                if kwargs.get('verbose', False):
                    print(f"Warning: fill_masks failed ({e}), using simple replacement")
                # Fallback: just replace <MASK> with empty string
                imputed_masked_sequence = masked_sequence.replace("<MASK>", "")

        # 2.) Compute the perplexity for each candidate with bounds checking
        ...
        # For very large candidate sets, add progress indication
        verbose_ppl = kwargs.setdefault('verbose', True) and len(candidates) < 50
        
        try:
            ppls = lm.perplexity(queries, return_as_list=True, verbose=verbose_ppl)
        except Exception as e:
            if kwargs.get('verbose', False):
                print(f"Warning: perplexity computation failed ({e}), using fallback")
            # Fallback: return random ordering
            import torch
            ppls = torch.rand(len(candidates)).tolist()
